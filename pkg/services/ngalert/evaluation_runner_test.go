...
func (c *testEvaluationCoordinator) ShouldEvaluate() bool {
...
func setupEvaluationLoopWithSchedule(t *testing.T, coordinator *testEvaluationCoordinator, sched *testSchedule, opts ...alertNGOption) *evaluationLoop {
	...
	go func() { done <- ng.runEvaluationLoop(ctx) }()
...
func TestEvaluationLifecycle_SchedulerExitsUnexpectedly(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("lifecycle should have returned an error but it kept running")
	}
}
...
func TestEvaluationRunner_PersisterSavesOnStart(t *testing.T) {
	...
	realPersister := state.NewSyncStatePersisiter(log.NewNopLogger(), stateManagerCfg)
	stateManager := state.NewManager(stateManagerCfg, realPersister)
	...
	// Process eval results to trigger a save
	...
	require.Greater(t, saveCount, 0, "persister should save data")
